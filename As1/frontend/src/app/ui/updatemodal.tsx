...
const UpdateModal = ({ prn, name }) => {
  const [modalIsOpen, setIsOpen] = useState(false);
  const [prn1, setPrn] = useState(prn);
  const [name1, setName] = useState(name);

  const handleSubmit = () => {
    axios.post("http://localhost:3000/update", { prn: prn1, name: name1 });
  }
    ...
    <div>
      ...
      >
        ...
        <form className="w-full max-w-lg flex flex-col">
          ...
          <button className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded " onClick={handleSubmit}> Update </button>
        </form>
      </Modal>
    </div>
  );
